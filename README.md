...
- The clickable project preview renders the actual page at the same viewport and language as the opened projection, scaled down without colour filters. Pedestal images are captured from that page in German/English and desktop/mobile sizes; regenerate them with `npm run build:example-previews` while Vite is running.
...
- Die klickbare Projektvorschau zeigt die tatsächliche Seite in derselben Sprache und mit denselben Ansichtsmaßen wie die geöffnete Projektion, verkleinert und ohne Farbfilter. Die Sockelbilder werden aus dieser Seite für Deutsch/Englisch und Desktop/Mobil erzeugt. Bei laufendem Vite lassen sie sich mit `npm run build:example-previews` aktualisieren.
