...
try {
  for(const [width,height] of [[1440,900],[1236,754],[768,1024],[390,844],[364,624]]) for(const lang of ['de','en']) {
    ...
    assert.equal(requests.some(url=>/terminal[.-]|@xterm/.test(url)),false,'Terminal is lazy-loaded');
    ...
    assert.equal(await page.locator('.stage svg').getAttribute('data-link'),'failing');
    assert(await page.locator('.log-lines .error').count()>0);
    await page.waitForFunction(()=>document.querySelector('.stage svg').dataset.link==='backup');
    assert.match(await page.locator('.log-lines').innerText(),/uplink B forwarding/);
    await page.locator('[data-cable]').click();assert.equal(await page.locator('.stage svg').getAttribute('data-link'),'primary');
    await go(2);
    ...
    await go(4);
    await page.keyboard.press('Control+k');await page.locator('.xterm-helper-textarea').waitFor({state:'attached'});
    await page.waitForFunction(()=>document.querySelector('.terminal-status').textContent==='');
    await page.keyboard.type('whoami');await page.keyboard.press('Enter');
    await page.waitForFunction(()=>document.querySelector('.xterm-screen').textContent.includes('TIEFGANG\\jana'));
    await page.keyboard.type('ipconfig');await page.keyboard.press('Enter');
    await page.waitForFunction(()=>document.querySelector('.xterm-screen').textContent.includes('10.20.0.42'));
    await page.keyboard.press('Tab');
    assert(await page.evaluate(()=>document.querySelector('.terminal-dialog').contains(document.activeElement)),'Terminal retains keyboard focus');
    await page.keyboard.press('Escape');assert(!await page.locator('.terminal-dialog').isVisible());
    ...
    await page.locator('[data-terminal]').click();await page.keyboard.type('ipconfig');await page.keyboard.press('Enter');
    await page.waitForFunction(lang=>document.querySelector('.xterm-screen').textContent.includes(lang==='de'?'Noch keine Netzwerkverbindung.':'No network connection yet.'),lang);
    await page.keyboard.press('Escape');
    await page.locator('#example-language').click();assert.equal(await page.locator('html').getAttribute('lang'),lang==='de'?'en':'de');
    assert.deepEqual(errors,[]);await page.close();console.log(`PASS ${width} × ${height} / ${lang}: chapters, DHCP, failover, VM, terminal, completion`);
  }
  for(const lang of ['de','en']) {
    ...
    assert.equal(await page.locator('.chapter-content svg').count(),7);
  ...
  assert.equal(await nojs.locator('.chapter-content svg').count(),14);
