...
- The clickable project preview renders the actual page at the same viewport and language as the opened projection. Its resting colours are muted; hover and keyboard focus reveal the original colours. Pedestal images are captured from that page in German/English and desktop/mobile sizes; regenerate them with `npm run build:example-previews` while Vite is running.
