...
const base=process.env.EXAMPLE_URL||'http://127.0.0.1:5176';
...
try {
 ...
 for(const [width,height] of [[1920,1080],[1440,900],[1366,768],[768,1024],[390,844]]){
  ...
  for(const language of ['de','en']){
   ...
   const measured=await page.evaluate(()=>{
    ...
    const panel=document.querySelector('.projects-panel'),rect=panel.getBoundingClientRect(),body=panel.querySelector('.cv-hologram__body');
    const delta=Object.fromEntries(Object.keys(projected).map(k=>[k,Math.abs(projected[k]-rect[k])]));
    return {delta,overflow:body.scrollWidth>body.clientWidth,scrollbar:getComputedStyle(body).scrollbarWidth,projectedHeight:rect.height,scrollable:body.scrollHeight>body.clientHeight,viewportFits:rect.left>=c.left-1&&rect.right<=c.right+1&&rect.top>=c.top-1&&rect.bottom<=c.bottom+1};
   });
   assert(Object.values(measured.delta).every(v=>v<1),JSON.stringify({width,height,language,...measured}));
   assert.equal(measured.overflow,false);assert.equal(measured.scrollbar,'none');assert(measured.viewportFits);
   await page.screenshot({path:`${output}/${width}-${language}.png`});results.push({width,height,language,...measured});
  }
 }
 const body=page.locator('.projects-panel .cv-hologram__body');await body.focus();await page.keyboard.press('End');
 await page.waitForFunction(()=>{const e=document.querySelector('.projects-panel .cv-hologram__body');return e.scrollTop>0||e.scrollHeight<=e.clientHeight;});
 await page.locator('[data-project-route="projekte/privat"]').click();await page.locator('.projects-soon').waitFor();
 ...
 await writeFile(`${output}/results.json`,JSON.stringify(results,null,2));console.log('10 DE/EN viewport checks: all four HTML/particle edges within 1px; no scrollbar/overflow; keyboard scroll, tabs, projection and ESC passed.');
