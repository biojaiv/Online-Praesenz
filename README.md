...
- Hovering a header section gently raises its pedestal and triggers two brief light pulses. All three hologram surfaces use an opaque dark blue matched to the scene; their text and images remain visible. The middle hologram disappears during the camera turn into the projected page.
- Clicking the middle hologram unfolds two opaque wings: amber Websites and cyan Systems integration. Both wings open their content directly: Jana’s Tiefgang website and the Recovery Lab film. There is no divider or section switch. Up/Down and the wheel retain camera zoom; mobile presents both sheets as scrollable HTML. All holograms share one physical height and jet alignment, and opening a pedestal keeps the same content and lighting. Pages and video load only when selected.
- Rendering uses device hints and sustained frame timings to limit GPU resolution, with lighter post-processing on constrained devices. Fully unlit Orrery meshes are skipped before drawing; conservative bounds retain every potentially illuminated surface. The 3D scene and its decorative update loop pause behind an opened website. Hidden preview documents and the tunnel canvas are released when closed; its contours draw around the page. HTML remains at native resolution. See the [measurement notes](docu_workprogress/resource-usage-2026-09-24.md) and [MDN's WebGL guidance](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API/WebGL_best_practices).
- The projected page occupies 80% of the desktop viewport, capped at 1400 px. Four corner rails and seven stationary, progressively less rounded ribs form a tunnel. Amber impulses travel along fine blue fibres towards the page; a warm glow and blurred preview reflection light the rear walls and floor. Subtle pointer parallax also works over the embedded page. Mobile shows only rails and glow; reduced motion and hidden tabs stop animation. Check with `npm run test:tunnel`.

## Two project wings and Recovery Lab

`src/data/projects.js` defines two entries: Tiefgang (Jana’s first day) under Websites and Recovery Lab under Systems integration. The earlier KnallBlauMedia concepts are no longer listed on the pedestal. The CV again uses the original `CV_Projection_DE/EN.webp` assets; the later `CV_Hologram` variants are no longer displayed. All visible pedestals retain their labels in the same style. The wheel zooms; hold the left mouse button or Shift while wheeling to scroll a document. `npm run test:hologram-alignment` checks geometry, unchanged preview content, labels and direct activation.
...
The Websites wing includes `/beispiel/?lang=en` (German: `?lang=de`). Seven scroll chapters follow Jana’s laptop from unboxing to a ready workplace. A lightweight SVG infrastructure drawing shares state with the DHCP dialogue, redundant uplink simulation, VM drawer and chapter-aware diagnostic terminal (⌘K / Ctrl+K). The terminal loads only when opened; commands run in a browser simulation.

The 00:04:12 counter is a compressed model timeline, not a measured deployment result. PXE and UEM chapters link to the actual baramundi pilot project. Reduced motion enables a reading view with seven still drawings; the generated HTML also contains complete English and German chapters without JavaScript. ESC closes the terminal or VM first, then returns from the projection.
...
- Beim Hover über einen Bereich in der Kopfzeile hebt sich der passende Sockel leicht und zeigt zwei kurze Lichtimpulse. Alle drei Hologrammflächen haben ein deckendes Dunkelblau, das zur Szene passt; Texte und Bilder bleiben sichtbar. Während der Kameradrehung zur projizierten Seite verschwindet das mittlere Hologramm.
- Beim Klick auf das mittlere Hologramm klappen zwei deckende Flügel auf: Webseiten in Amber und Systemintegration in Cyan. Beide Flügel öffnen ihre Inhalte direkt: Janas Tiefgang-Webseite und den Recovery-Lab-Film. Trennlinie und Umschalter entfallen. Hoch-/Runterpfeil und Mausrad zoomen weiterhin; mobil sind beide Blätter untereinander scrollbar. Alle Hologramme haben dieselbe Höhe und denselben Übergang zum Jetstream. Beim Öffnen bleiben Inhalt und Beleuchtung gleich. Seiten und Film laden erst beim Auswählen.
- Gerätehinweise und dauerhaft langsame Bildzeiten begrenzen automatisch die GPU-Auflösung. Schwächere Geräte erhalten eine leichtere Nachbearbeitung. Vollständig unbeleuchtete Orrery-Flächen werden vor dem Zeichnen ausgelassen; großzügige Objektgrenzen erhalten alle möglicherweise beleuchteten Flächen. Hinter einer geöffneten Webseite pausieren die 3D-Szene und ihre zusätzlichen Animationen. Verborgene Vorschau-Dokumente und die Tunnel-Canvas werden beim Schließen freigegeben; die Konturen liegen um die Webseite. HTML bleibt in nativer Auflösung. Details stehen in den [Messnotizen](docu_workprogress/resource-usage-2026-09-24.md).
- Die projizierte Seite nimmt am Desktop 80 % der Fensterbreite ein, höchstens 1400 px. Vier Eckkanten und sieben stehende, nach hinten weniger gerundete Rippen bilden einen Tunnel. Amberimpulse wandern auf feinen blauen Fasern zur Seite; warmer Schein und eine unscharfe Vorschau-Spiegelung beleuchten die hinteren Wände und den Boden. Dezente Mausparallaxe funktioniert auch über der eingebetteten Seite. Mobil bleiben nur Kanten und Schein; reduzierte Bewegung und verborgene Tabs stoppen die Animation. Prüfung mit `npm run test:tunnel`.

## Zwei Projektflügel und Recovery Lab

`src/data/projects.js` enthält nur Tiefgang/Jana unter Webseiten und Recovery Lab unter Systemintegration. Die früheren KnallBlauMedia-Konzepte werden am Sockel nicht mehr angeboten. Der Lebenslauf verwendet wieder die ursprünglichen Dateien `CV_Projection_DE/EN.webp`; die später erzeugten `CV_Hologram`-Varianten werden nicht mehr angezeigt. Alle sichtbaren Sockel behalten ihre Beschriftungen im gleichen Stil. Das Mausrad zoomt; mit gehaltener linker Maustaste oder Umschalt blättert es im Dokument. Der alte Pfad `projekte/privat` führt jetzt zur Systemintegration.
...
Der mittlere Sockel öffnet `/beispiel/?lang=de` (Englisch: `?lang=en`). Sieben Scrollkapitel begleiten Janas Laptop vom Auspacken bis zum fertigen Arbeitsplatz. SVG-Zeichnung, DHCP-Gespräch, Uplink-Ausfall, VM-Schublade und kapitelabhängiges Diagnose-Terminal teilen sich einen Zustand. Das Terminal (⌘K / Strg+K) wird erst beim Öffnen geladen; alle Befehle laufen als Simulation im Browser.

00:04:12 ist eine verdichtete Modellzeit, kein gemessenes Deployment-Ergebnis. PXE und UEM verweisen auf das echte baramundi-Pilotprojekt. Reduzierte Bewegung aktiviert eine Lesefassung mit sieben Standbildern; auch ohne JavaScript enthält das erzeugte HTML alle Kapitel auf Englisch und Deutsch. ESC schließt zuerst Terminal oder VM, danach die Projektion.
